...
static uint8_t weekday_to_bin(uint8_t weekday)
{
  return 1 << weekday;
}

static uint8_t bin_to_weekday(uint8_t bin_weekday)
{
...
void RTC::getTime(struct tm *rtc_time)
{
...
void RTC::writeRTC(byte reg, byte value){
    Wire.beginTransmission(RTC_ADDR);
    Wire.write(reg);
    Wire.write(value);
    Wire.endTransmission();
