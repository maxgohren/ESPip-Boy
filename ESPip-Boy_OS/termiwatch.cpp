...
static void draw_termiwatch()
{
  ...
  // if item state changes, make call to change corresponding display state
    ...
    /* TEMP */
    WeatherData currentWeather = WeatherService_getWeather();
    // TODO only update when weather data changes, not every second
    snprintf(textLine, sizeof(textLine), "[TEMP] %.1f C", currentWeather.temperature);
...
static void DisplayUpdateTask(void *parameter)
{
